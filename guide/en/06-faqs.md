...
There may be situations where this optimization is not desired, in which case you can turn it off via the [`output.hoistTransitiveImports`](#outputhoisttransitiveimports) option. This optimization is also never applied when using the [`preserveModules`](#preservemodules) option.
...
2. Otherwise, additionally making the polyfill a separate entry or [manual chunk](#manualchunks) will always make sure it is executed first.
